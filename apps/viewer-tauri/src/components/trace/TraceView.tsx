...
export function TraceView() {
  ...
  const trace_last_seq = useStore((s) => s.trace_last_seq);
  ...
  if (!selected_task_id) {
    return <EmptyState icon={Route} heading="Select a task" description="Choose a task to view trace." />;
  }
  ...
  return (
    <div className="w-full h-full overflow-y-auto custom-scrollbar px-6 py-6 space-y-4">
      ...
      {entries.length === 0 ? (
        <EmptyState icon={Route} heading="No trace yet" description="Trace entries will appear here as the agent works." />
      ) : (
        <Timeline entries={entries} showPayload />
      )}
    </div>
  );
}
