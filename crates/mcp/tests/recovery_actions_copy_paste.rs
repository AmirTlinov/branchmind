...
fn assert_recovery_command_is_schema_valid(server: &mut Server, id: i64, line: &str) {
    ...
    assert!(
        ...
        "recovery command must not contain placeholder markers (<...>): {trimmed}"
    );
...
#[test]
fn recovery_actions_copy_paste_valid_for_tasks_snapshot_unknown_id() {
    let mut server = Server::start_initialized_with_args(
        "recovery_actions_copy_paste_valid_for_tasks_snapshot_unknown_id",
        &["--toolset", "daily"],
    );
    ...
    let resp = tools_call(
        ...
        "tasks",
        json!({
            "workspace": "ws_cp_unknown_id",
            "op": "call",
            "cmd": "tasks.snapshot",
            "args": { "task": "TASK-99999999", "view": "smart" },
            "fmt": "lines"
        }),
    );
    let rendered = extract_tool_text_str(&resp);
