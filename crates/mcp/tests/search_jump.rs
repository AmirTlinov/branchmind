...
fn search_tools_return_open_actions() {
        ...
        "params": {
            ...
            "arguments": {
                ...
                "args": {
                    "anchor": "core",
                    "key": "jump-search",
                    "card": { "title": "Jump Search", "text": "hello" }
                }
            }
        }
    }));
    ...
    let ksearch = server.request(json!({
        "jsonrpc": "2.0",
        "id": 5,
        "method": "tools/call",
        "params": { "name": "think", "arguments": { "workspace": "ws_search_jump", "op": "knowledge.search", "args": {
            "text": "jump-search",
            "limit": 5
        } } }
    }));
    let ksearch = extract_tool_text(&ksearch);
    assert_eq!(ksearch.get("success").and_then(|v| v.as_bool()), Some(true));
    let items = ksearch
        .get("result")
    ...
    assert!(
        items
            .iter()
            .any(|it| it.get("card_id").and_then(|v| v.as_str()) == Some(card_id.as_str())),
        "items must include the card"
    );
    ...
    assert!(
        ...
        "knowledge.search must include open(id=CARD-..) action"
    );
}
