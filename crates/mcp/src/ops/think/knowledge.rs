...
use super::knowledge_lint;
use super::{KB_BRANCH, KB_GRAPH_DOC, KB_TRACE_DOC};

pub(super) fn handle_knowledge_upsert(server: &mut crate::McpServer, env: &Envelope) -> OpResponse {
...
pub(super) fn handle_knowledge_key_suggest(
...
pub(super) fn handle_knowledge_query(server: &mut crate::McpServer, env: &Envelope) -> OpResponse {
    let Some(ws) = env.workspace.as_deref() else {
        return OpResponse::error(
            env.cmd.clone(),
            OpError {
...
pub(super) fn handle_knowledge_recall(server: &mut crate::McpServer, env: &Envelope) -> OpResponse {
    let Some(ws) = env.workspace.as_deref() else {
        return OpResponse::error(
            env.cmd.clone(),
            OpError {
...
pub(super) fn handle_knowledge_lint(server: &mut crate::McpServer, env: &Envelope) -> OpResponse {
...
pub(super) fn normalized_claim(title: Option<&str>, text: Option<&str>) -> String {
