...
impl McpServer {
    pub(crate) fn tool_tasks_jobs_proof_attach(&mut self, args: Value) -> Value {
        ...
        if checks.is_empty() && attachments.is_empty() {
            ...
            if let Some(w) = unknown_warning.clone() {
                warnings.push(w);
            }
            return ai_ok_with_warnings("tasks_jobs_proof_attach", result, warnings, Vec::new());
        }
        ...
        if let Some(w) = unknown_warning {
            ai_ok_with_warnings("tasks_jobs_proof_attach", result, vec![w], Vec::new())
        } else {
