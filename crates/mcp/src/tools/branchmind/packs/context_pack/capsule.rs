...
pub(super) fn build_context_pack_capsule(args: ContextPackCapsuleArgs<'_>) -> Value {
    ...
    json!({
        "type": "context_pack_capsule",
        "version": 1,
        "where": {
            "workspace": args.workspace.as_str(),
            "branch": args.branch,
            "docs": { "notes": args.notes_doc, "trace": args.trace_doc, "graph": args.graph_doc },
            "lane": lane,
...
pub(super) fn filter_context_pack_capsule_to_cards(capsule: &mut Value, cards_snapshot: &[Value]) {
