...
planfs_v1:
  ...
  objective: tasks.planfs.init/import/export implemented per contract
  status: todo
  budgets:
    max_files: 16
    max_diff_lines: 1500
  ...
  dod:
    success_criteria:
    - tasks.planfs.init/import/export implemented per contract
    - Strict import rejects placeholders/empty criteria/tests/blockers and enforces budgets
    - import→export idempotent when unchanged
    - Verify passes
    tests:
    - make check
    blockers:
...
- make check
## Proof
- FILE:docs/plans/flagship-ux-v2-planfs-no-knowledge-golden-surface/Slice-3.md
## Rollback
- Rollback slice 3 changes.
## Risks
