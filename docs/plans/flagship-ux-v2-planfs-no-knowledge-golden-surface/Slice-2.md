...
planfs_v1:
  ...
  objective: system.cmd.list default shows golden only; mode=all shows full registry
  status: todo
  budgets:
    max_files: 16
    max_diff_lines: 1500
  ...
  dod:
    success_criteria:
    - system.cmd.list default shows golden only; mode=all shows full registry
    - system.schema.list default shows golden only; mode=all shows full registry
    - Docs (QUICK_START/tutorial) updated
    - Verify passes
    tests:
    - make check
    blockers:
...
- make check
## Proof
- FILE:docs/plans/flagship-ux-v2-planfs-no-knowledge-golden-surface/Slice-2.md
## Rollback
- Rollback slice 2 changes.
## Risks
