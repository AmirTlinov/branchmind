...
- In **session modes** (stdio + shared proxy), hot reload is enabled by default as a DX optimization.
- In `--daemon` mode, hot reload is disabled by default (daemons are long-lived; opt-in if desired).
